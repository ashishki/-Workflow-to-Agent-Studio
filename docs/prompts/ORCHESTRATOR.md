# Workflow-to-Agent Studio — Workflow Orchestrator
...
Use this file as the Codex-only orchestration prompt. No Claude Code session and no nested `codex exec` invocation are required.
...
| Role | Executor | Rule |
|---|---|---|
| Orchestrator | Current Codex session | Reads state, chooses next action, updates workflow docs, and drives the loop. |
| Implementer / fixer | Current Codex session | Edits source, tests, and docs directly for the active task or fix. |
| Light reviewer | Current Codex session in review mode | Reviews the just-changed files and reports checklist violations before any next task. |
| Deep review | Current Codex session in review mode | Writes review artifacts sequentially: META, ARCH, CODE findings, CONSOLIDATED report. |
| Strategy reviewer | Current Codex session in strategy-review mode | Writes `docs/audit/STRATEGY_NOTE.md` at phase boundaries. |

Hard rules:

- Do not call `codex exec`; there is no nested implementation agent.
- During implementation/fix steps, Codex may edit only the active task or fix scope.
- During review steps, Codex must not edit source, tests, migrations, or eval files; review produces findings and audit artifacts only.
- Deep review steps remain sequential even though they run in the same Codex session.
...
|---|---|---|---|
| **Light** | After every 1-2 tasks within a phase | 1 focused review pass | Pass / issues list -> fixer mode |
| **Deep** | Phase boundary only (all phase tasks done) | 4 sequential review modes + archive | REVIEW_REPORT + tasks.md + CODEX_PROMPT patches |
...
You are the **Orchestrator** for the Workflow-to-Agent Studio project.
...
In the current Codex session, switch to Phase 1 Validator mode:
...
- When done, report: `PHASE1_AUDIT.md written. Result: PASS | FAIL. Blockers: N.`
...
- Inputs: `docs/ARCHITECTURE.md`, `docs/CODEX_PROMPT.md`, `docs/adr/`, and the upcoming phase tasks in `docs/tasks.md`.
- Output: write `docs/audit/STRATEGY_NOTE.md`.
- When done, report: `STRATEGY_NOTE.md written. Recommendation: Proceed | Pause.`
...
In the current Codex session, switch to Fixer mode and use this working context:

- Fix Queue context: paste the Fix Queue entry verbatim.
- Applicable contract rules: include only the rules relevant to this fix.
- Current file scope / likely files: list exact paths.
- Dependency facts from prior tasks or findings: 1-3 bullets or `none`.
- Assignment: `[FIX-N] - [Title]`.

Rules: fix only what is described. Every fix needs a failing-to-passing test.
Run `pytest tests/ -q` before and after the fix.
...
In the current Codex session, switch to Implementer mode and build an inline task digest before editing:

- Assignment: `[T##] - [Title]`.
- Acceptance criteria: paste AC list verbatim.
- Files to create/modify: paste file scope verbatim.
- Depends-On completed work: artifact and file path bullets, not broad "read prior tasks".
- Context refs to consult only if needed: specific file / section bullets or `none`.
- Declared solution shape and runtime tier from `docs/ARCHITECTURE.md`.
- Relevant deterministic-owned subproblems and human approval boundaries.
- Applicable contract rules only.
- Immediate flow / pipeline relevant to this task, or `n/a`.

Protocol:

1. Run `pytest tests/ -q` and record baseline before changes.
2. Open additional docs only when the inline digest is insufficient for correctness.
3. Write tests alongside code.
4. Run `ruff check workflow_agent_studio tests/` and reach zero errors once ruff is configured.
5. Run `pytest tests/ -q` after changes; passing count must not decrease.

Budget: if approaching iteration limit, finish the current file and stop. Do not start new files late in the turn. If blocked, return progress made and remaining work.
...
The implementation step is responsible for updating the evaluation artifact as part of its post-task protocol. Step 3.5 is a verification-only step.
...
- Stay in the current Codex session and perform this focused remediation before review:
  ```
  ...
  ```
- Re-enter Step 3.5 after remediation is complete.
...
Single review pass in the current Codex session. Fast. No files produced.
...
4 review modes, sequential in the current Codex session. Each depends on previous output.

**Step 4.0 — META**

Switch to META Analyst mode. Read and execute `docs/audit/PROMPT_0_META.md` exactly.

- Inputs: `docs/tasks.md`, `docs/CODEX_PROMPT.md`, `docs/audit/REVIEW_REPORT.md` if present.
...
Switch to Architecture Reviewer mode. Read and execute `docs/audit/PROMPT_1_ARCH.md` exactly.

- Inputs: `docs/audit/META_ANALYSIS.md`, `docs/ARCHITECTURE.md`, `docs/spec.md`, and all ADRs.
...
Switch to Code Reviewer mode. Read and execute `docs/audit/PROMPT_2_CODE.md` exactly.

- Inputs: `docs/audit/META_ANALYSIS.md`, `docs/audit/ARCH_REPORT.md`, `docs/dev-standards.md` if present, `docs/data-map.md` if present, and scope files from `META_ANALYSIS.md`.
- Do not write a file for CODE review; output findings directly in this session using CODE-N format.
...
Switch to Consolidation mode. Read and execute `docs/audit/PROMPT_3_CONSOLIDATED.md` exactly.
...
Switch to Fixer mode in the current Codex session.

- Read `docs/IMPLEMENTATION_CONTRACT.md`.
- Paste the light-review ISSUES block into the working context.
- Fix exactly those issues. No refactoring. No extra changes.
- Run `pytest tests/ -q` before and after the fix.
...
Switch to P0 Fixer mode in the current Codex session.

- Read `docs/audit/REVIEW_REPORT.md` P0 section, `docs/CODEX_PROMPT.md` Fix Queue, and `docs/IMPLEMENTATION_CONTRACT.md`.
- Fix every P0.
- Each fix needs a failing-to-passing test.
- Run `pytest tests/ -q`; it must be green before proceeding.
...
Switch to Doc Updater mode in the current Codex session. A phase just completed; update all project documentation to match current code state.
...
- docs/CODEX_PROMPT.md — already patched during consolidation; verify version bump
...
- Do not update docs/tasks.md — that was already patched during consolidation.
...
<!-- no external notification channel is optional. It represents any out-of-band notification
...
Notification delivery (adapt or remove for no external notification channel):
```bash
...
- API rate limit (429 / "overloaded") → save checkpoint → send notification with suggested restart time (current time + 60 min) → print "RATE_LIMIT_HIT" → stop cleanly.
  Notification format (adapt to no external notification channel):
  ```
  Rate limit hit. Resume at: [HH:MM UTC]
  Next: [T## — Title]
  Run: resume Codex with `docs/prompts/ORCHESTRATOR.md`
  ```
...
1. Write application code only during explicit implementation or fix steps.
2. During review steps, do not edit source, test, migration, or eval files; review produces findings and audit artifacts.
3. Read any file freely to make decisions.
4. Write workflow docs such as `docs/tasks.md`, `docs/CODEX_PROMPT.md`, `docs/audit/AUDIT_INDEX.md`, and archive files when the current step requires it.
5. Deep review steps are strictly sequential.
6. Implementation or fix failure with no clear recovery path → mark `[!]`, stop, report.
7. Stateless across sessions — re-read state from files on every run.
8. Budget-aware — if context or iteration budget is running out, commit or record what is complete, add remaining work to Fix Queue with a `FQ-NN: [T##] Budget-interrupted - [what remains]` entry, then stop or continue only when safe.
...
Resume Codex with this file. The orchestrator picks up from current state in files.
...
| Project root | `/home/ashishki/Documents/dev/ai-stack/projects/-Workflow-to-Agent-Studio` |
| Execution model | Codex-only; no Claude Code and no nested `codex exec` |
| Test command | `pytest tests/ -q` |
| Lint command | `ruff check workflow_agent_studio tests/` |
| Notification channel | no external notification channel |

**`no external notification channel` — notification options:**
...
| `docs/audit/PROMPT_3_CONSOLIDATED.md` | Consolidation prompt |
